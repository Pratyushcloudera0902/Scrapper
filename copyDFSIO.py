...
FILE_NAME = "heythere" + str(int(time.time()))
...
def DFSIO(nr_files, file_size, job_type, user, jar_loc):
...
def createDirectory():  # done
    command_formation = f"sudo -u systest ozone fs -mkdir -p ofs://ozone1/testing/benchmarks/"
    stdin, stdout, stderr = c.exec_command(command_formation)
    time.sleep(5)
    return stdout, stderr
...
def main_exec(user, jar_loc):
    ...
    stderr, stdout = DFSIO('2', '16KB', 'write', user, jar_loc)
    ...
    stderr1, stdout1 = DFSIO('2', '16KB', 'read', user, jar_loc)
...
def main():
    ...
    if new_fs == str_ozone:
        # Clean the directory
        removeDirectory("sudo -u systest ozone fs -rm -r -skipTrash ofs://ozone1/testing/")
